...
OUTPUT_DIR = PROJECT_ROOT / "releases" / "windowsstore" / "screenshots-launchboards"
...
def _board_entries(items: list[tuple[Path, str]]) -> list[dict[str, str | None]]:
    ...
    # Vorrang vor path) -- reale lokale Installationspfade (inkl. Windows-
    # Benutzername) waeren sonst im Screenshot lesbar, siehe die gleiche
    # Korrektur in generate_store_screenshots.py (T-20260926-510472849).
...
def generate_store_screenshots(output_dir: Path) -> list[Path]:
    ...
    with tempfile.TemporaryDirectory(prefix="softwarecenter-store-shots-") as temp_dir:
        ...
        app.setApplicationName("LaunchBoards Store Screenshots")
        ...
        builder = MainWindow(settings=builder_settings, profile=PROFILE_LAUNCHBOARDS)
        ...
        window = MainWindow(settings=settings, profile=PROFILE_LAUNCHBOARDS)
